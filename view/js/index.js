...
function Initialize(){
    ...
    $.ajax({
    	...
    	// 最初にhtml()に設定しておく
    	//===============================================
    	var $input = $('<input type="checkbox" />').attr({'name':'checkbox', 'id': 'categoryid_'+categorydata[0].id, 'value':categorydata[0].id, 'onchange':'searchCategory();'}).prop('checked',true).removeAttr('disabled');
    	...
    	for(var index = 1; index < categorydata.length; index++){
    		var $input = $('<input type="checkbox" />').attr({'name':'checkbox', 'id': 'categoryid_'+categorydata[index].id, 'value':categorydata[index].id, 'onchange':'searchCategory();'}).prop('checked', true).removeAttr('disabled');
